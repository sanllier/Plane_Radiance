...
static INLINE void float2rgbe(unsigned char rgbe[4],
                              float red,
                              float green,
                              float blue)
{
...
static INLINE void rgbe2float(float* red,
                              float* green,
                              float* blue,
...
/* stride - number of bytes between start of successive elements */
int RGBE_WritePixels(FILE* fp, float* data, int stride, int numpixels)
{
  unsigned char rgbe[4];
...
int _RGBE_WritePixels(FILE* fp, XYZ** data, int x, int y)
{
  unsigned char rgbe[4];

  for (int i = 0; i < x; ++i) {
      for (int q = 0; q < y; ++q) {
          float2rgbe(rgbe, data[i][q].x_, data[i][q].y_, data[i][q].z_);
          if (fwrite(rgbe, sizeof(rgbe), 1, fp) < 1)
              return rgbe_error(rgbe_write_error, NULL);
      }
  }
...
/* stride - number of bytes between start of successive elements */
int RGBE_ReadPixels(FILE* fp, float* data, int stride, int numpixels)
{
  unsigned char rgbe[4];
                         ...
                         int num_scanlines)
{
  ...
  /* run length encoding is not allowed so write flat*/
    return RGBE_WritePixels(fp, data, stride, scanline_width * num_scanlines);
                        ...
                        int num_scanlines)
{
  ...
  /* run length encoding is not allowed so read flat*/
    return RGBE_ReadPixels(fp, data, stride, scanline_width * num_scanlines);
  ...
  while (num_scanlines > 0)
  {
    ...
    if ((rgbe[0] != 2) || (rgbe[1] != 2) || (rgbe[2] & 0x80))
    {
      ...
      data += stride;
      free(scanline_buffer);
      return RGBE_ReadPixels(fp, data, stride, scanline_width * num_scanlines - 1);
    }
